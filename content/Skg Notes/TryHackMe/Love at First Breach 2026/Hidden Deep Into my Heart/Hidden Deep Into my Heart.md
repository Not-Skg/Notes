...
![[Vault.png|500]]
A première vu rien d'intéressant.
En essayant des mettre des mots clés à la suite de l'url on peut potentiellement trouver des pages cachés. Essayons avec robots.txt, c'est un fichier qui se positionne à la racine d'un site et sert à dire au robots de ne pas fouiller à certains endroits. Généralement c'est surtout pour empêcher l'indexation des pages admin par exemple.
![[robots.png|500]]
...
Allons voir à quoi ressemble la page caché : 
![[secret_vault.png|500]]
Bon, ça ne nous avance pas trop, essayons donc de savoir s'il y a une page de connexion lié à cet url, pour ce faire on pourrait tenter des mots clés aux hasards (ex: connexion, portail, check_id, etc) mais pour aller plus vite on va utiliser l'outil gobuster, il va faire ce travail a notre place.
![[admin.png|500]]
Il a déjà trouvé une page nommé "administrator", allons voir a quoi elle ressemble.
![[admin2.png|500]]
...
On peut donc trouver la page de connexion et s'y connecter facilement grace à ces secrets.
## Pour s'en protéger
