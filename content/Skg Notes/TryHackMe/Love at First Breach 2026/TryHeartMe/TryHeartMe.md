...
## Retex
Сette fois-ci on va devoir analyser une boutique en ligne.
![[Shop.png|500]]
On peut commencer par check les pages associés a cette boutique via l'outil `gobuster`.
![[Gobuster.png|500]]
Il ne donne pas grand chose d'intéressant, que des pages classique (login, register, admin, etc)

Vu qu'on a la possibilité de se créer un compte, on va essayer et regarder a quoi ressemble la requête qui s'envoie en parallèle.
![[T_CA.png|500]]
...
![[cookie.png|500]]
En le décodant on peut voir qu'il contient l'email, le role, le nombre de crédit, un iat et le thème de la page.
...
Donc pour l'instant on va essayer de créer notre propre token avec pleins de crédit, le role d'admin et l'ago `none`.
...
(ne pas oublier le secret à la fin, parce qu'un token est composé : d'un header, d'une payload et d'un secret, c'est la partie bleu du screen du dessus)

![[nouveau_produit.png|500]]
En modifiant la requête reçu avec le nouveau token jwt on a pleins de crédits et on peut voir un nouveau produit "ValenFlag".
(Pour chaque interaction sur le site il faudras s'assurer d'avoir le bon token)
...
## Pour s'en protéger
- **Vérif serveur stricte** : Toujours revalider `role`, `credits` en base de données côté serveur (ne JAMAIS faire confiance au JWT seul)
