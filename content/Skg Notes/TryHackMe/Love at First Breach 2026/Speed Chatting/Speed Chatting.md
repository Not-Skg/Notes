...
D'après l'énoncé, les devs ont été pressé et la plateforme n'a pas été proprement testé. Étant donné que la plateforme est centralisé sur la sécurisation de la privatisation des messages, ils n'ont peut être pas passé beaucoup de temps sur la gestion de l'upload des photos de profil.
...
Mais grâce au script, toutes les commandes que je note dans mon terminal se font aussi du coté serveur, j'ai donc un reverse shell. En affichant les fichiers disponibles, je peux voir le flag.txt qui contient : **==THM{R3v3rs3_Sh3ll_L0v3_C0nn3ct10ns}==**
...
- *Check fichiers uploads :* Vérifier chaque fichier upload avant d'en faire quoi que ce soit, via un antivirus comme clamscan par exemple 
